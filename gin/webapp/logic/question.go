...
// CreateQuestion CreateQuestion创建问题
...
func GetQuestionList(offset, limit int) (apiQuestionList *models.ApiQuestionList, err error) {
	...
	//涉及到大量数掘的slice.如果知道数据的数量,应当使用make预先初始化
	//比append()扩容效率更高
...
// GetQuestionDetail GetQuestionDetail获取问题详情
func GetQuestionDetail(questionID uint64) (data *models.ApiQuestionDetail, err error) {
	...
	if !ok {
		zap.L().Error("get categoryMap failed", zap.Error(err))
		return
	}
