...
func GetQuestionList(offset, limint int) (questionList []*models.Question, err error) {
	sqlStr := `select
                     question_id, caption, content, author_id, category_id, create_time
             from
             question order by id desc
             limit ?,?`
	err = db.Select(&questionList, sqlStr, offset, limint)
	if err != nil {
...
func GetQuestionCount() (answerCount int, err error) {
	sqlStr := `select count(question_id) from question`
...
func GetQuestion(questionID uint64) (question *models.Question, err error) {
	...
	sqlStr := `select question_id, caption, content, author_id, category_id, create_time 
