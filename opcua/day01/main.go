...
func main() {
	var (
		//	endpoint = flag.String("endpoint", "opc.tcp://milo.digitalpetri.com:62541/milo", "OPC UA Endpoint URL")
		...
		interval = flag.Duration("interval", opcua.DefaultSubscriptionMaxKeepAliveCount, "subscription interval")
	)
	...
	c := opcua.NewClient("opc.tcp://milo.digitalpetri.com:62541/milo", opts...)
	...
	wg.Add(1)
	go startCallbackSub(ctx, m, *interval, time.Second*3, wg, *nodeID)
...
//)
//
//func cleanup(sub *monitor.Subscription, wg *sync.WaitGroup, ctx context.Context) {
//	log.Printf("stats: sub=%d delivered=%d dropped=%d", sub.SubscriptionID(), sub.Delivered(), sub.Dropped())
//	sub.Unsubscribe(ctx)
//	wg.Done()
//}
...
//	// start callback-based subscription
//	wg.Add(1)
//	go startCallbackSub(ctx, m, 10, 10, wg, nodeID)
//
//	<-ctx.Done()
//	wg.Wait()
//}
